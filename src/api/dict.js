  ...
  export const updateDictTypeStatus = (id, status) => {
    return request.patch(`dict-type/${id}/status`, { status })
  }
  ...
  export const getDictDataList = (dictTypeId, params) => {
    return request.get(`/dict-data/code/${dictTypeId}`, { params })
  }
  ...
  export const getDictDataDetail = (id) => {
    return request.get(`/dict-data/detail/${id}`)
  }

  // 更新字典数据状态
  export const updateDictDataStatus = (id, status) => {
    return request.patch(`/dict-data/${id}/status`, { status })
  }

  // 根据字典编码获取字典数据（前端使用）
  export const getDictByCode = (code) => {
    return request.get(`/dict-data/${code}`)
  }